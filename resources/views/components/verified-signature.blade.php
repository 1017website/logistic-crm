@props(['signatureQr', 'verificationUrl', 'signerName', 'signerTitle', 'companyName', 'signerPhone' => null, 'language' => 'id'])
@once
<style>
...
<table class="verified-signature">
    <tr>
        ...
        <td class="copy-cell">
            <div class="signed-by">{{ $language === 'en' ? 'This document is electronically signed by:' : 'Dokumen ini ditandatangani secara elektronik oleh:' }}</div>
            <div class="signer-name">{{ $signerName }}</div>
            <div class="signer-title">{{ $signerTitle }}</div>
            @if(filled($signerPhone))
                <div class="signer-phone">No. HP: {{ $signerPhone }}</div>
            @endif
            <div>{{ strtoupper($companyName) }}</div>
            <div class="verify-copy">{{ $language === 'en' ? 'Scan the QR code to verify the authenticity of this document.' : 'Pindai QR untuk memverifikasi keaslian dokumen.' }}</div>
        </td>
    </tr>
</table>
